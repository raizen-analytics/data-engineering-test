...
class Database(object):
    ...
    def insert(self, table, fields, values):
        ...
        insertQuery = insertQuery.format(table,','.join(fields),
                                            ','.join(len(fields)*'?'))
        cursor = self.conn.cursor()
    ...
    def index(self, table, column):
        indexName = '{0}_{1}'.format(table,column)
        indexQuery = 'CREATE INDEX {0} ON {1} ({2})'
        indexQuery = indexQuery.format(indexName,table,column)
        cursor = self.conn.cursor()
        cursor.execute(indexQuery)
        logging.info('index {0} created on {1}'.format(indexName,table))
        self.conn.commit()
        cursor.close()
